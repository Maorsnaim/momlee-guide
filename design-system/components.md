...
| Slider + Control handle | `302:4779` + `302:4960` (↳ Sliders) | — | — | ⛔ blocked | the whole set binds the OLD UI-kit token layer, not Momlee variables: track `bg-quaternary`, progress `fg-brand-primary-(500)` #b05f64, handle `components/sliders/*` #8d4c50 (absent from the export), labels `text-primary-(900)`/`_alt` names — build starts when Maor rebinds + a fresh export lands (from-sivan 2026-08-16 night) |
