...
| Badge group | `506:13337` (2.0 `Badge group` set) | — | — | ❌ dropped | Maor 2026-08-17: legacy node, NOT built on 2.0 variables and not in use — "what is not in use should not go into development". The real Badge is `730:9618` (already built ✅). Never build this node. |
...
| BottomSheet | `483:8499` (2.0 `Bottom Sheet`, ↳ Overlays) | `title` · `helpText` (Figma `Show Help Text`) · `showTopButtons` · `onBack`/`backLabel` · `onSkip`/`skipLabel` ("דילוג") · `children` (the Content slot) · `onNext`/`nextDisabled`/`nextLoading`/`nextLabel` | — | ✅ `apps/web` (2026-08-16) | the PANEL only (overlay/open-close = the screen): rounded-t-4xl, px-3xl py-xl, min-h-356, column gap-xl; composes Button ×3 (back = sm neutral-solid icon, skip = xs link, next = sm brand-solid icon — the master shows next Disabled); title text-md Medium tracking-text-half + optional text-sm help; ⚠ raw values in the master: sheet bg raw white (bound to same-valued bg-surface) + drag handle raw #d0d5dc (nearest token border-secondary #d1d1d1 used) — raw-values family. The `Popup` frame (818:26618) on the page is a non-component anatomy example of this sheet — no separate build |
| Divider | `14:31` (Horizontal; vertical twin `14:24` — ↳ Dividers/Cursuers) | `orientation: horizontal\|vertical` | — | ✅ `apps/web` (2026-08-16) | 1px border-subtle hairline (stroke read from both masters' SVG assets, #f3f3f3); components whose own masters carry an edge keep drawing it themselves |
| Cursor | `242:1274` (CursorShown/Hidden) | — (a style binding, not a component) | — | ✅ via `Input` (2026-08-16) | the 1×22 brand caret (#b37a81 = fg-brand-primary) → web = the PLATFORM caret (`caret-fg-brand-primary` on Input's element; OTP hides its native input entirely so no caret). ⚠ the master carries annotation attributes the MCP didn't surface — if the caret has logic beyond color, Maor should say |
| Filters | `179:4243` (↳ Controls/View) | `active` · `count` (the bubble) · `onClick` · `label` | — | ✅ `apps/web` (2026-08-16) | 30px box, 18px sliders-horizontal glyph (stroke 1.5 per the master); Active = rounded-full ring in border-strong at the master's raw 1.4px width + the sm NumberBubble pinned −6px top/start |
| ViewType | `179:4261` (↳ Controls/View) | `view: list\|map` · `onToggle` · `label` | — | ✅ `apps/web` (2026-08-16) | shows the view you can SWITCH TO (List → map glyph; Map → list-sort-ascending glyph + ring); the Map ring is raw black in the master vs the Filters sibling's border-strong — bound to border-strong for consistency, noted. The page's `iOS Keyboard` set (425:4110) = system-UI reference frames, NO build |
| Slider + Control handle | `302:4779` + `302:4960` (↳ Sliders) | — | — | ❌ dropped | Maor 2026-08-17 verbatim: he does not recall designing sliders and they are not relevant — the page is legacy UI-kit content, removed from DS scope. Never build. |
